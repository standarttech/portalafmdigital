-- Drop and recreate all audit triggers to ensure they exist
...
DROP TRIGGER IF EXISTS audit_budget_plans ON public.budget_plans;
DROP TRIGGER IF EXISTS audit_client_targets ON public.client_targets;
...
CREATE TRIGGER audit_budget_plans AFTER INSERT OR UPDATE OR DELETE ON public.budget_plans FOR EACH ROW EXECUTE FUNCTION public.log_audit_event();
CREATE TRIGGER audit_client_targets AFTER INSERT OR UPDATE OR DELETE ON public.client_targets FOR EACH ROW EXECUTE FUNCTION public.log_audit_event();