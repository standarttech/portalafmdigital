    ...
    <div>
      <section className="py-24 sm:py-32 px-4 sm:px-6 text-center">
        <motion.p initial="hidden" animate="visible" variants={fadeUp} custom={0}
          className="text-[hsl(42,87%,55%)] text-xs sm:text-sm font-semibold tracking-[0.2em] sm:tracking-[0.3em] uppercase mb-3 sm:mb-4">{t('cases.badge')}</motion.p>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6 text-center border-t border-white/5">
        <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">{t('cases.wantResults')}</h2>
        <p className="text-white/50 mb-6 sm:mb-8 max-w-lg mx-auto text-sm">{t('cases.wantResultsDesc')}</p>
        <Link to="/contact"
          ...
          {t('cases.bookAudit')} <ArrowRight className="h-4 w-4 sm:h-5 sm:w-5" />
        </Link>
      </section>
    </div>
  );
