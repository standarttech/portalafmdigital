...
export default function CrmLeadsPage() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const { data } = await supabase.from('crm_leads').select('*').order('created_at', { ascending: false }).limit(500);
      setLeads(data || []);
      setLoading(false);
    })();
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter(l => l.full_name?.toLowerCase().includes(q) || l.email?.toLowerCase().includes(q) || l.phone?.toLowerCase().includes(q) || l.company?.toLowerCase().includes(q));
  }, [leads, search]);
