...
interface Route {
  ...
  priority: number; is_active: boolean; timeout_seconds: number; retry_limit: number; created_at: string;
}
...
export default function AiInfraRoutesPage() {
  ...
  const [form, setForm] = useState({ task_type: '', primary_provider_id: '', fallback_provider_id: '', timeout_seconds: 60, retry_limit: 1 });

  const load = useCallback(async () => {
    const [rRes, pRes] = await Promise.all([
      supabase.from('ai_provider_routes' as any).select('*').order('task_type'),
      supabase.from('ai_providers' as any).select('id, name, slug').eq('is_active', true).order('name'),
    ]);
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 pt-2">
              <div><Label>Task Type</Label><Input value={form.task_type} onChange={e => setForm(f => ({ ...f, task_type: e.target.value }))} placeholder="e.g. ai_ads_analysis" /></div>
              <div><Label>Primary Provider</Label>
              ...
              <Button onClick={createRoute} disabled={!form.task_type || !form.primary_provider_id || saving} className="w-full">
                {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null} Create Route
              </Button>
            </div>
          </DialogContent>
        </Dialog>
              ...
              <CardContent className="p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
                    <span className="font-medium">{providerName(r.primary_provider_id)}</span>
                    {r.fallback_provider_id && (
                      <><ArrowRight className="h-3 w-3" /><span>{providerName(r.fallback_provider_id)}</span></>
                    )}
                    <span>·</span>
