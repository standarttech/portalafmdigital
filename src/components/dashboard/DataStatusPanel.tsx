...
async function fetchSyncData(isAdmin: boolean): Promise<SyncItem[]> {
  ...
  const data = pcData || [];
  if (data.length === 0 && !metaApiLastSyncAt) return [];
...
export default function DataStatusPanel({ isAdmin }: Props) {
  ...
  const { data: syncData = [] } = useQuery({
    queryKey: ['dashboard-sync-status', isAdmin],
    queryFn: () => fetchSyncData(isAdmin),
    staleTime: 3 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
  });
