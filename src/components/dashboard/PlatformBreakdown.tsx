...
import type { PlatformFilter } from './dashboardData';
import type { PlatformDataPoint } from '@/hooks/useDashboardMetrics';
...
export default function PlatformBreakdown({ platformData, onPlatformChange }: Props) {
  const { t, formatCurrency } = useLanguage();

  const avgCpl = useMemo(() => {
    const withLeads = platformData.filter(d => d.leads > 0);
  ...
  return (
    <Card className="glass-card h-full">
      ...
      <CardContent>
        {platformData.length === 0 || platformData.every(p => p.spend === 0) ? (
          <div className="flex items-center justify-center h-[200px] text-muted-foreground text-sm">{t('common.noData')}</div>
        ) : (
          <>
            <div className="h-[200px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={platformData} cx="50%" cy="50%" innerRadius={55} outerRadius={80} paddingAngle={4} dataKey="spend">
            ...
            <div className="mt-3">
              ...
              {platformData.map((p) => {
                ...
                const cplColor = cpl === 0 ? 'text-muted-foreground' : cplRatio < 0.9 ? 'text-success' : cplRatio > 1.1 ? 'text-destructive' : 'text-warning';
                return (
                  <button key={p.name} onClick={() => onPlatformChange(p.key as PlatformFilter)}
                    ...
                    <span className={`text-right font-medium font-mono ${cplColor}`}>{cpl > 0 ? formatCurrency(cpl) : '—'}</span>
                  </button>
                );
              })}
            </div>
