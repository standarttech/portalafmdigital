...
import { getAlerts } from './dashboardData';
...
export default function AttentionRequired({ filters }: Props) {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const alerts = useMemo(() => getAlerts(filters), [filters]);
      ...
      <div className="space-y-1">
        {alerts.map((alert) => {
          ...
          return (
            <button
              key={alert.id}
              onClick={() => alert.clientId ? navigate(`/clients/${alert.clientId}`) : navigate('/sync')}
