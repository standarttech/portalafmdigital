...
export default function PerformanceChart({ filters, className }: Props) {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    const raw = rawData.find(d => d.date === label);
    if (!raw) return null;
    return (
      <div className="bg-card border border-border rounded-lg p-3 shadow-lg text-xs space-y-1">
        <p className="font-semibold text-foreground mb-1">{label}</p>
        {visible.spend && (
          <p style={{ color: metrics[0].color }}>{t('dashboard.spend')}: {formatCurrency(raw.spend)}</p>
        )}
        {visible.leads && (
          <p style={{ color: metrics[1].color }}>{t('dashboard.leads')}: {formatNumber(raw.leads)}</p>
        )}
        {visible.cpl && (
          <p style={{ color: metrics[2].color }}>{t('dashboard.cpl')}: {formatCurrency(raw.cpl)}</p>
        )}
      </div>
    );
  };
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {metrics.map(m => (
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 text-xs font-medium px-2 py-1 rounded-md transition-opacity',
                  visible[m.key as keyof typeof visible] ? 'opacity-100' : 'opacity-30'
                )}
              >
