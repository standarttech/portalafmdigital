...
function buildTelegramLeadTemplate(formFields: FormField[]): string {
  ...
  const customFields = formFields.filter(f => !standardSlugs.has(f.slug));
...
export default function FbLeadFormSetupWizard({ automationId, metaConns, triggerConfig }: Props) {
  ...
  const [historicalDays, setHistoricalDays] = useState<number | null>(null);
  const [historicalLoading, setHistoricalLoading] = useState(false);
  const [historicalResult, setHistoricalResult] = useState<{ success: boolean; leads_fetched?: number; leads_processed?: number; error?: string } | null>(null);
