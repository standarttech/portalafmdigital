...
const ScalingStackLanding: React.FC = () => {
  ...
  // Init Meta Pixel on landing page too
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    utmKeys.forEach((key) => { const val = params.get(key); if (val) result[key] = val; });
    ...
    <div className="ss-root overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-14 sm:pt-16 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-5 sm:px-6 text-center py-16 sm:py-24">
          ...
          <div className="flex items-center justify-center mb-10 sm:mb-14" style={{ animation: "ss-fade-up 0.7s 0.3s ease both" }}>
      ...
      {/* ── STATS ── */}
      <section className="relative" style={{ background: "rgba(99,179,237,0.04)" }}>
        <div ref={statsRef} className="max-w-5xl mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-2 md:grid-cols-4">
            {STATS.map((s, i) => (
              <div key={s.label} className="relative">
                {/* Vertical divider (not on first of row) */}
                {i > 0 && (
                  <div className="absolute left-0 top-[20%] bottom-[20%] w-px hidden md:block" style={{ background: "rgba(99,179,237,0.12)" }} />
                )}
                {i % 2 !== 0 && (
                  <div className="absolute left-0 top-[20%] bottom-[20%] w-px md:hidden" style={{ background: "rgba(99,179,237,0.12)" }} />
                )}
                <StatCard value={s.value} label={s.label} trigger={statsInView} />
              </div>
            ))}
          </div>
        </div>
